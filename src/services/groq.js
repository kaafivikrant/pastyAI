...
class GroqService {
  ...
  async isAvailable() {
    try {
      const apiKey = this.config.getApiKey('groq');
      if (!apiKey) {
  ...
  async processText(text, mode = null) {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        const errorMsg = error.response?.data?.error?.message || 'Invalid request to Groq API';
  ...
  buildPrompt(systemPrompt, userText) {
    ...
    // Groq uses the messages format, so this is mainly for logging
  ...
  async testConnection() {
    try {
      ...
      if (!isAvailable) {
        return { success: false, message: 'Cannot connect to Groq API' };
      }
      ...
      if (!modelExists) {
        return { 
          success: false, 
          message: `Model "${currentModel}" not found. Available models: ${models.map(m => m.id).join(', ')}` 
        };
      }
      ...
      return {
        success: true,
        message: `Connected to Groq successfully. Model: ${currentModel}`,
